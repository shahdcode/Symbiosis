...
class _HomeBody extends StatelessWidget {
  ...
  int get _attentionCount =>
      myPlants.where((p) => p.status == PlantStatus.attention).length;
    ...
    return SafeArea(
      child: CustomScrollView(
        ...
        slivers: [
          ...
          if (showNotification)
            SliverToBoxAdapter(child: _buildNotification()),
      ...
      child: Container(
        ...
        child: Row(
          children: [
            ...
            const Expanded(
              child: Text(
                'Calathea needs water soon — 36 min',
                style: TextStyle(
                    fontSize: 12,
  ...
  Widget _buildSummaryPills() {
    final thriving =
        myPlants.where((p) => p.status == PlantStatus.thriving).length;
    return Padding(
      padding: const EdgeInsets.fromLTRB(24, 16, 24, 0),
      child: Row(
        children: [
          _Pill(label: '${myPlants.length} Plants',
              bg: AppTheme.lightGreen, text: AppTheme.primaryGreen),
          const SizedBox(width: 8),
          _Pill(label: '$thriving Thriving',
              bg: AppTheme.lightGreen, text: const Color(0xFF16A34A)),
          const SizedBox(width: 8),
          _Pill(label: '$_attentionCount Attention',
    ...
    return SizedBox(
      ...
      child: ListView.separated(
        ...
        itemBuilder: (_, i) => PlantCard(
          plant: myPlants[i],
          onTap: () => onNavigatePlant(myPlants[i]),
          index: i,
        ),
      ),
    );
...
class _Pill extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      child: Text(label,
          style:
              TextStyle(fontSize: 12, color: text, fontWeight: FontWeight.w600)),
    );
  }
}
